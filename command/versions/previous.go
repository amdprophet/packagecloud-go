...
func PreviousVersionCommand(getClientFn packagecloud.GetClientFn) *cobra.Command {
	var repo packagecloud.Repo
	var name string

	cmd := &cobra.Command{
		Use:   "previous <user/repo> <package name>",
		Short: "Show the previous version of a package with a given name in a given repository",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			previous, err := client.PreviousVersion(options)
			if err != nil {
